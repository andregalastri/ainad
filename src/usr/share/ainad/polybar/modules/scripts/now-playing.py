...
def update_prefix_suffix(player_name="", status=""):
    ...
    suffix = "     %{F#525660}%{T4}|%{T-}%{F-}     " + prev_button
    if status == "Playing":
        suffix += " "+pause_button
        status_paused = False
    else:
    ...
    suffix += " "+next_button
    # print(suffix)
    display_prefix = suffix #+ "  %{F#525660}%{T4}|%{T-}%{F-}  "

    #for key in display_player_prefix.keys():
    #    if key in player_name:
    #        display_suffix = display_player_prefix[key]
    #        break
    #else:
    #    display_suffix = display_player_prefix["default"]
...
def print_text():
    ...
    if hide_output and (len(players)==0 or display_text==" No title - No artist |"):
        print("", flush = True)
        return
    scroll()
